...
public class CicloHandler {
    ...
    public void programar(Map<?, ?> cicloMap, long scheduledTicks, List<FabricTask> currentTasks, String targetWorld, boolean usePapi, net.minecraft.server.network.ServerPlayerEntity triggerPlayer) {
        ...
        if (pasoDelayStr == null || (comandosPlantilla == null && comandosJugadorPlantilla == null)) {
            Autotask.LOGGER.warn("Acción de ciclo mal configurada (falta paso_delay o comandos)");
            return;
        }

        if (variantes == null || variantes.isEmpty()) {
            if (pasosFijos > 0) {
                programarCicloSimple(comandosPlantilla, comandosJugadorPlantilla, scheduledTicks, TimeUtils.parseTimeToTicks(pasoDelayStr), pasosFijos, currentTasks, targetWorld, usePapi, triggerPlayer);
            } else {
                Autotask.LOGGER.warn("Ciclo sin 'variantes' y sin 'pasos' definidos");
            }
            return;
        }
        ...
        for (int i = 0; i < maxSteps; i++) {
            ...
            if (comandosPlantilla != null) {
                ejecutarBucleComandos(comandosPlantilla, i, pasoStr, iStr, maxStepsStr, placeholders, listasDeValores, cadaN, modoRangos, esAleatorio, finalDelay, currentTasks, targetWorld, usePapi, false, triggerPlayer);
            }
            if (comandosJugadorPlantilla != null) {
                ejecutarBucleComandos(comandosJugadorPlantilla, i, pasoStr, iStr, maxStepsStr, placeholders, listasDeValores, cadaN, modoRangos, esAleatorio, finalDelay, currentTasks, targetWorld, usePapi, true, triggerPlayer);
            }
            currentLoopDelay += pasoDelayTicks;
        }
    }
    ...
    private void ejecutarBucleComandos(List<?> comandos, int i, String pasoStr, String iStr, String maxStepsStr,
                                       ...
                                       long finalDelay, List<FabricTask> currentTasks, String targetWorld, boolean usePapi, boolean asPlayer, net.minecraft.server.network.ServerPlayerEntity triggerPlayer) {
        for (Object cmdPlantillaObj : comandos) {
                ...
                if (cmdMap.containsKey("bucle")) {
                    ...
                    if (vecesObj != null) {
                        ...
                        try {
                            ...
                            Autotask.LOGGER.warn("Valor de 'veces' inválido en bucle: " + vecesStr);
                            veces = 0;
                        }
                    } else {
                        veces = 0;
            ...
            for (int k = 0; k < veces; k++) {
                ...
                currentTasks.add(TickScheduler.runTaskLater(() ->
                                TaskManager.dispatchCommand(comandoReal, targetWorld, usePapi, asPlayer, triggerPlayer),
                        finalDelay));
            }
        }
    }
    ...
    private void programarCicloSimple(List<?> comandos, List<?> comandosJugador, long startDelayTicks, long pasoDelayTicks, int pasos, List<FabricTask> taskList, String targetWorld, boolean usePapi, net.minecraft.server.network.ServerPlayerEntity triggerPlayer) {
        ...
        for (int i = 0; i < pasos; i++) {
            ...
            if (comandos != null) {
                ejecutarBucleSimpleComandos(comandos, iStr, pasoStr, maxStepsStr, finalDelay, taskList, targetWorld, usePapi, false, triggerPlayer);
            }
            if (comandosJugador != null) {
                ejecutarBucleSimpleComandos(comandosJugador, iStr, pasoStr, maxStepsStr, finalDelay, taskList, targetWorld, usePapi, true, triggerPlayer);
            }
            currentLoopDelay += pasoDelayTicks;
        }
    }

    private void ejecutarBucleSimpleComandos(List<?> comandos, String iStr, String pasoStr, String maxStepsStr, long finalDelay, List<FabricTask> taskList, String targetWorld, boolean usePapi, boolean asPlayer, net.minecraft.server.network.ServerPlayerEntity triggerPlayer) {
    ...
    private List<String> parseRango(String rango) {
        ...
        if (rango.contains(",")) {
            ...
            try {
                ...
                Autotask.LOGGER.warn("Formato de rango inválido en ciclo: " + rango);
            }
        } else {
    ...
    private boolean evaluarCondicion(String condicion) {
        ...
        try {
            ...
            Autotask.LOGGER.warn("Error al evaluar condición de ciclo: " + condicion);
            return false;
        }
        return false;
    }
